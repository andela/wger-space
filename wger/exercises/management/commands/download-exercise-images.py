...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option(
            ...
            help=" \
            Remote URL to fetch the exercises from (default: https://wger.de)",
        ),
    )
    ...
    def handle(self, **options):
        ...
        # Get all exercises
        result = requests.get(
            exercise_api.format(remote_url), headers=headers
        ).json()
        for exercise_json in result["results"]:
            ...
            # Get all images
            images = requests.get(
                image_api.format(remote_url, exercise_id), headers=headers
            ).json()
            ...
            if images["count"]:
                ...
                for image_json in images["results"]:
                    ...
                    image.image.save(
                        os.path.basename(image_name), File(img_temp)
                    )
                    image.save()
