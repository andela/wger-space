...
class Ingredient(AbstractLicenseModel, models.Model):
    ...
    def clean(self):
        ...
        if self.energy:
            ...
            if not (
                (energy_upper > energy_calculated)
                and (energy_calculated > energy_lower)
            ):
                raise ValidationError(
                    _(
                        "Total energy is not the approximate sum of the"
                        " energy provided by protein, carbohydrates and fat."
                    )
                )
...
class IngredientWeightUnit(models.Model):
    ...
    def __str__(self):
        ...
        return u"{0}{1} ({2}g)".format(
            self.amount if self.amount > 1 else "", self.unit.name, self.gram
        )
