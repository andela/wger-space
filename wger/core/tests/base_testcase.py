...
class WorkoutManagerAddTestCase(WorkoutManagerTestCase):
    """
    Tests adding an object as an authorized user, a different one and
    a logged out one.
    """
    ...
    fileupload = None
    """
    If the form requires a file upload, specify the field name and
    the file path here in a list or tuple:
    ...
    def add_object(self, fail=False):
        ...
        count_before = self.object_class.objects.count()
        self.pk_before = (
            self.object_class.objects.all().order_by("id").last().pk
        )
