...
def get_current_settings(exercise, set_id):
    ...
    function in the template, and we are only interested on the
...
def pagination(paginator, page):
    ...
    # than 8000 items) we muck around here to remove the
    # pages not inmediately 'around' the current one, otherwise we end up
    # with a useless block with 300 pages.
