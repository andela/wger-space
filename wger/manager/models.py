...
class ScheduleManager(models.Manager):
    ...
    def get_current_workout(self, user):
        ...
        # there are no active schedules, just return the last workout
            ...
            schedule = False
            try:
                active_workout = Workout.objects.filter(user=user).latest(
                    "creation_date"
                )
...
class Day(models.Model):
    ...
    def get_canonical_representation(self):
        ...
        for set_obj in self.set_set.select_related():
            ...
            # drop the rest, because otherwise it doesn't make sense
    ...
    def get_workout_session(self, date=None):
        ...
        try:
            return WorkoutSession.objects.filter(
                user=self.user).get(workoutlog_id=self.pk)
    ...
    def clean(self):
        ...
        if (not self.time_end and self.time_start) or (
            ...
            raise ValidationError(
                _(
                    "If you enter a time, you must enter both start and end \
                    time."
                )
            )
