...
def create(request, day_pk):
    ...
    if request.flavour == "mobile":
        ...
        form.fields[
            "exercise_list"
        ].queryset = Exercise.objects.accepted().filter(language__in=languages)
...
def create_dropset(request, day_pk):
    ...
    # Other context we need
        ...
        context["formsets"] = formsets
        context["form_action"] = reverse(
            "manager:set:drop_set", kwargs={"day_pk": day_pk}
        )
        context["extend_template"] = (
            "base_empty.html" if request.is_ajax() else "base.html"
        )
        return render(request, "set/drop_set.html", context)
...
def get_formset(request, exercise_pk, reps=Set.DEFAULT_SETS):
    ...
    return render(
        request, "set/formset.html", {"formset": formset, "exercise": exercise}
    )
