...
def add(request, pk):
    ...
    # We need several lists here because we need to assign specific form
    #  to each exercise: the entries for weight and repetitions have no
    #  indicator to which exercise they belong besides the form-ID,
    # from Django's formset
    counter = 0
    total_sets = 0
    exercise_list = {}
    form_to_exercise = {}
        ...
        for day_list in self.object.canonical_representation["day_list"]:
            ...
            for set_list in day_list["set_list"]:
                ...
                for exercise_list in set_list["exercise_list"]:
                    ...
                    #        reason (bugin django? DB problems?) when adding
                    #        the filter there, the execution time explodes.
                    #        The weight unit filter works as expected.
                    #        Also, adding the unit IDs to the exclude list
                    #        also has the disadvantage that if new ones are
                    #        added in a local instance, they could "slip"
                    #        through.

                    logs = (
                        exercise_list["obj"]
                        .workoutlog_set.filter(
